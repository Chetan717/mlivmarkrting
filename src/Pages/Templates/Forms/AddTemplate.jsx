  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center gap-6 p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/60 dark:bg-gray-800/40">
        ...
        <span
          className={`ml-auto px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${value ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400" : "bg-red-50 text-red-500 dark:bg-red-500/10 dark:text-red-400"}`}
        >
...
export default function AddTemplate() {
  ...
  const handleSelectTypeChange = useCallback((e) => {
    ...
    setForm((p) => ({
      ...p,
      SelectType: val,
      Date: val === "Festival" ? p.Date : "",
    }));
  }, []);

  const selectTypeOptions = useMemo(
    () => getSelectTypes(form.MainType),
    [form.MainType],
  );
  ...
  const handleSubmit = useCallback(
    async (e) => {
      e.preventDefault();
      if (!form.MainType || !form.SelectType) {
        setError("Main Type and Select Type are required.");
        toast.danger("Failed Main save template. Please try again.");
        return;
      }
      if (!form.Showcase_url) {
        setError("Showcase URL is required.");
        toast.danger("Showcase URL is required.");
        return;
      }
      setLoading(true);
      setError(null);
      try {
        ...
        await addDoc(collection(db, "mlmtemplate"), {
          ...form,
          serial: Number(form.serial) || 0,
          GraphicsLink: cleanGraphics,
        ...
        toast.danger("Failed to save template. Please try again.");
      } finally {
        setLoading(false);
      }
    },
    ...
    <div className="p-4 md:p-6 max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <SelectField
              ...
              placeholder={
                form.MainType ? "Select type…" : "Choose Main Type first"
              }
              required
              disabled={!form.MainType}
            />
          </div>
          ...
          {isFestival && (
            <div className="flex flex-col gap-1.5 p-3 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
              ...
              <p className="text-xs text-amber-600 dark:text-amber-400">
                Required for Festival type templates
              </p>
            </div>
          )}
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-5 space-y-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <ImageUploadInput
              ...
              onChange={setField("Showcase_url")}
              storagePath="templates/showcase"
              placeholder="Paste URL or click ↑ to upload showcase image"
            />
          </div>
          <div className="flex flex-col gap-1.5">
            <FieldLabel>Form Showcase Image URL</FieldLabel>
            <ImageUploadInput
              value={form.ShowCaseForm}
              onChange={setField("ShowCaseForm")}
              storagePath="templates/showcase-form"
              placeholder="Paste URL or click ↑ to upload form showcase image"
            />
            <p className="text-xs text-gray-400">
              Used as the in-app form background / preview image
            </p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-5 space-y-4">
          ...
          <ToggleSwitch
            label="Active"
            description="Template is visible and active"
            checked={form.Active}
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-5">
          ...
          <p className="text-xs text-gray-400 mt-3">
            Fields shown / hidden automatically based on{" "}
